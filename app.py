...
def recognize_face(uploaded_img):
    ...
    for index, row in employees.iterrows():
        try:
            ...
            if result["verified"]:
                return row["id"], f"Match found with distance {result['distance']:.3f}"
...
def mark_attendance(emp_id):
    ...
    if not attendance[
        (attendance["id"] == emp_id) & (attendance["date"] == today)
    ].empty:
...
if menu == "Register Employee":
    ...
    if st.button("Register Employee"):
        if not uploaded or not name or not emp_id:
            ...
            success, msg = register_employee(name, emp_id, img)
...
# ---------------------------------------------------
    ...
    if uploaded:
        ...
        if st.button("Recognize & Mark Attendance"):
            ...
            if emp_id:
                st.success(msg)
                success, att_msg = mark_attendance(emp_id)
                st.success(att_msg) if success else st.warning(att_msg)
                st.balloons()
            else:
                st.error(msg)
